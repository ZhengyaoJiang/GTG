...
from agent.util import unique_list
...
def str2clause(s):
    ...
    if len(atoms) ==2:
        ...
        for strs in [var_string(atom) for atom in body+[head]]:
            var_strs.extend(strs)
        return clause.replace_by_dict({s: i for i,s in enumerate(unique_list(var_strs))})
    else:
        return Clause(head, [])
...
class Clause():
    ...
    @property
    def variables(self):
        return unique_list(sum([atom.variables for atom in self.atoms], []))

    @property
    def constants(self):
        return unique_list(sum([atom.constants for atom in self.atoms], []))

    @property
    def predicates(self):
        return unique_list([atom.predicate for atom in self.atoms])
