...
wall_color = [0, 0, 0]
all_colors = {**{19:agent_color, 20:goal_color, 21:grid_color, 22:wall_color}, **colors}
color2index = {tuple(c): i for i, c in all_colors.items()}
all_colors = [all_colors[i] for i in range(len(all_colors))]
...
def world_gen(n=12, goal_length=3, num_distractor=2, distractor_length=2, seed=None):
    ...
    world = np.ones((n, n, 3), dtype=np.int32) * 220
    ...
    # add black wall
    wall_0 = np.zeros((1, n, 3), dtype=np.int32)
    wall_1 = np.zeros((n+2, 1, 3), dtype=np.int32)
