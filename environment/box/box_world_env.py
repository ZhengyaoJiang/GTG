...
class BoxWorld(gym.Env):
    ...
    def __init__(self, n, goal_length, num_distractor, distractor_length, max_steps=10**4,
                 collect_key=True, world=None, render_key=True):
        ...
        self.observation_space.spaces["image"] = Box(low=0, high=255, shape=(n+2, n+2, 3), dtype=np.uint8)
    ...
    def step(self, action):
        ...
        return {"image": self.world}, reward, done, info
    ...
    def get_index_img(self):
        world = self.world.astype(np.int32)
        width, height, _ = world.shape
        ind_img = np.zeros([width, height], dtype=np.int32)
        for x in range(width):
            for y in range(height):
                ind_img[x,y] = color2index[tuple(world[x,y])]
        return ind_img
